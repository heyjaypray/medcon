                              ...
                              <div className="event-schedule d-flex bg-white rounded p-3 border">
                                <div className="float-left">
                                  <ul className="date text-center text-primary mr-md-4 mr-3 mb-0 list-unstyled">
                                    <li className="day font-weight-bold mb-2">{moment(event.StartDate).format('DD')}</li>
                                    <li className="month font-weight-bold">{moment(event.StartDate).format('MMM')}</li>
                                  </ul>
                                </div>
                                <div className="content">
                                  <h4><Link to="#" className="text-dark title">{show.Name}</Link></h4>
                                  <p className="text-muted location-time"><span className="text-dark h6">Speakers:</span> {event.speakers.map(i => {
                                    return <p>{i.Name}</p>;
                                  })} <span className="text-danger">{show.isLive ? 'Live' : null}</span> <br /> <span className="text-dark h6">Time:</span>{show.StartTime.substring(0, show.StartTime.length - 7)} - {show.EndTime.substring(0, show.EndTime.length - 7)}</p>
                                  {show.isLive ? <Link to="#tickets" className="btn btn-sm btn-outline-primary mouse-down">Join Live Meeting</Link>: null}
                                </div>
                              </div>
                            </Col>
                          )
